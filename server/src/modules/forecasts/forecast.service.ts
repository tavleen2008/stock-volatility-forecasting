import { fetchStockHistory, fetchStockMetrics } from '../stocks/stock.service';

export const ForecastService = {
    getVolatilityForecast: async (symbol: string) => {
        // Fetch 3-month history (approx. 60-65 trading days)
        const history = await fetchStockHistory(symbol, '3mo');
        ...
        const calculateSMA = (data: number[], period: number): number => {
            if (data.length < period) return data.length > 0 ? +(data.reduce((a, b) => a + b, 0) / data.length).toFixed(2) : 0;
        ...
        const calculateVolatility = (data: number[]): number => {
            if (data.length < 2) return 25.0; // standard default vol
